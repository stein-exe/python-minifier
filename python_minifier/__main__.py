...
def parse_args():
    ...
    minification_options.add_argument(
        '--target-version',
        type=str,
        help='Target Python version (for parser compatibility), e.g. 3.10',
        dest='target_version',
    )
    ...
    if args.target_version:
        parts = args.target_version.split('.')
        if len(parts) != 2 or parts[0] != '3' or not parts[1].isdigit():
            sys.stderr.write('error: --target-version must be in the form 3.x (e.g. 3.11)\n')
            sys.exit(1)
...
def do_minify(source, filename, minification_args):
    ...
    if rename_globals and len(minification_args.path) > 1:
        # Per-file minification cannot safely track cross-module symbol usage.
        # Disable global renaming in this mode to avoid breaking local imports.
    ...
    minified_result = minify(
        ...
        target_version=minification_args.target_version,
    )

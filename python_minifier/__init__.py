...
def _normalize_target_version(target_version):
    if target_version is None:
        return None

    if isinstance(target_version, tuple) and len(target_version) == 2:
        major, minor = target_version
        if major != 3:
            raise ValueError('target_version major must be 3')
        return int(minor)

    if isinstance(target_version, str):
        parts = target_version.strip().split('.')
        if len(parts) != 2:
            raise ValueError('target_version string must be like "3.11"')
        major, minor = int(parts[0]), int(parts[1])
        if major != 3:
            raise ValueError('target_version major must be 3')
        return minor

    raise TypeError('target_version must be None, tuple[int, int], or "3.x" string')
...
def minify(
    ...
    :param target_version: Target Python version as (3, minor) tuple or "3.minor" string, independent from runtime
    :type target_version: tuple[int, int] or str or None
    ...
    feature_version = _normalize_target_version(target_version)
    if feature_version is None:
        module = ast.parse(source, filename)
    else:
        module = ast.parse(source, filename, feature_version=feature_version)
    ...
    minified = unparse(module, prefer_single_line=prefer_single_line, target_version=target_version)
...
def unparse(module, prefer_single_line=False, target_version=None):
    """
    ...
    :param bool prefer_single_line: If semi-colons should be preferred over newlines where there is no difference in output size
    :param target_version: Target Python version as (3, minor) tuple or "3.minor" string
    :rtype: str
    ...
    try:
        ...
        if feature_version is None:
            ...
            minified_module = ast.parse(printer.code, 'python_minifier.unparse output', feature_version=feature_version)
